...
class AppInfoCard(SimpleCardWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.nameLabel = TitleLabel('Start GDB', self)
        ...
        self.descriptionLabel = BodyLabel(
            '一键启动gdb工具，如果提前选择了dump文件以及symbols也提供一键恢复现场', self)
...
class SettinsCard(GroupHeaderCardWidget):
    ...
    def __init__(self, parent=None, parentvBoxLayout=None):
        ...
        # 选择按钮以及输入框部件
        self.executablefilechooseButton = PushButton("选择")
        #self.fileLineEdit = LineEdit()
        self.coredumpfileButton = PushButton("选择")

        # 设置Button的点击事件
        self.executablefilechooseButton.clicked.connect(self.executablefilechooseButtonClicked)
        self.coredumpfileButton.clicked.connect(self.coredumpfilechooseButtonClicked)
        ...
        # 设置部件的固定宽度
        self.executablefilechooseButton.setFixedWidth(120)
        self.coredumpfileButton.setFixedWidth(120)
        ...
        self.executablefileGroup = self.addGroup("{}/images/Rocket.svg".format(resource_path), "executable文件路径", "请选择文件", self.executablefilechooseButton)
        self.coredumpfileGroup = self.addGroup("{}/images/jsdesign.svg".format(resource_path), "coredump文件路径", "请选择文件", self.coredumpfileButton)
    ...
    def coredumpfilechooseButtonClicked(self):
        ...
        if self.coredumpfile == "":
            self.coredumpfileButton.setText("选择")
            self.coredumpfileGroup.setContent("请选择output目录")
        else:
            # 设置chooseButton的文字显示已选择
            self.coredumpfileButton.setText("已选择")
            self.coredumpfileGroup.setContent(self.coredumpfile)


    def executablefilechooseButtonClicked(self):
        ...
        if self.executablefile == "":
            # 设置vmlinuxButton的文字显示已选择
            self.executablefilechooseButton.setText("选择")
            self.executablefileGroup.setContent("请选择image文件")
        else:
            self.executablefilechooseButton.setText("已选择")
            self.executablefileGroup.setContent(self.executablefile)
    ...
    def runButtonClicked(self):
        ...
        logger.info("executable file path: {}".format(self.executablefile))
        logger.info("coredump file path: {}".format(self.coredumpfile))
        ...
        #self.stateTooltip = StateToolTip('正在运行', '请耐心等待', self)
        # 状态提示放到中心位置
        ...
        #os.system("start cmd /k cd /d {} && start startGDB.bat {} {}".format(GDBTOOL_PATH, self.executablefile, self.coredumpfile))
        os.system(command)
...
class StartGDBCardsInfo(ScrollArea):
...
class StartGDBInterface:
    ...
    def addTab(self, routeKey, text, icon):
        ...
        # tab左对齐
        self.mainWindow.showInterface.addWidget(StartGDBCardsInfo(routeKey=routeKey))
        self.mainWindow.showInterface.setCurrentWidget(self.mainWindow.showInterface.findChild(StartGDBCardsInfo, routeKey))
        self.mainWindow.stackedWidget.setCurrentWidget(self.mainWindow.showInterface)

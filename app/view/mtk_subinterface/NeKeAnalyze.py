...
class AppInfoCard(SimpleCardWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.tagButton2 = PillPushButton('NE/KE', self)
...
class DescriptionCard(HeaderCardWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.descriptionLabel = BodyLabel(
            'NE/KE Analyze 是MTK平台开发提供给研发人员进行KE以及NE的解析db使用的一个工具。本软件直接集成，无需下载，直接选择dump文件进行解析即可！\n 解析步骤：\n'
            '1. 选择经过AEE extract解析出来的KE/NE的目录\n'
            '2. 选择run\n'
            '3. 执行结束后再解析出的output目录的src/symbols等目录下添加vmlinux符号表等\n'
            '4. 再次执行\n'
            '注意: 两次执行非必须，看您需要，如果需要更为细致的解析，两次更加！', self)
...
class SettinsCard(GroupHeaderCardWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.dbgroup = self.addGroup("{}/images/Rocket.svg".format(resource_path), "KE/NE日志目录", "请选择目录", self.dbchooseButton)
    ...
    def showNofileErrorFlyout(self):
        Flyout.create(
            icon=InfoBarIcon.ERROR,
            title='db dir is not choose',
            content="db目录未选择, 请检查后再执行",
            target=self.runButton,
            parent=self.window()
        )
    ...
    def ondbChooseButtonClicked(self):
        ...
        if self.dbdir == "":
            ...
            self.dbgroup.setContent("请选择目录")
        else:
            self.dbchooseButton.setText("已选择")
            self.dbgroup.setContent(self.dbdir)
    ...
    def customSignalHandler(self, value):
        ...
        # 打开解析完成的文件夹
        logger.info("output dir: {}".format(self.dbdir))
        os.system("start {}".format(self.dbdir))
    ...
    def onRunButtonClicked(self):
        ...
        elif not "KE.dbg.DEC" in self.dbdir and not "NE.dbg.DEC" in self.dbdir:
...
class NeKeAnalyzeCardsInfo(ScrollArea):
...
class NeKeAnalyzeInterface:
    ...
    def addTab(self, routeKey, text, icon):
        ...
        # tab左对齐
        self.mainWindow.showInterface.addWidget(NeKeAnalyzeCardsInfo(routeKey=routeKey))
        self.mainWindow.showInterface.setCurrentWidget(self.mainWindow.showInterface.findChild(NeKeAnalyzeCardsInfo, routeKey))

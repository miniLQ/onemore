...
class SettinsCard(GroupHeaderCardWidget):
    ...
    def __init__(self, parent=None, parentvBoxLayout=None):
        ...
        # Noc error list选择部件
        self.NocErrlistComboBox = ComboBox()

        # 入口脚本部件
        self.ERRLOG0_LOW_lineEdit = LineEdit()
        self.ERRLOG0_HIGH_lineEdit = LineEdit()

        self.ERRLOG1_LOW_lineEdit = LineEdit()
        self.ERRLOG1_HIGH_lineEdit = LineEdit()

        self.ERRLOG2_LOW_lineEdit = LineEdit()
        self.ERRLOG2_HIGH_lineEdit = LineEdit()

        self.ERRLOG3_LOW_lineEdit = LineEdit()
        self.ERRLOG3_HIGH_lineEdit = LineEdit()

        self.FAULTINSTATUS0_LOW_lineEdit = LineEdit()
        self.FAULTINSTATUS0_HIGH_lineEdit = LineEdit()
        self.FAULTINSTATUS1_LOW_lineEdit = LineEdit()
        self.FAULTINSTATUS1_HIGH_lineEdit = LineEdit()

        self.ERRLOG0_LOW_lineEdit.setFixedWidth(320)
        self.ERRLOG0_HIGH_lineEdit.setFixedWidth(320)
        self.ERRLOG1_HIGH_lineEdit.setFixedWidth(320)
        self.ERRLOG1_LOW_lineEdit.setFixedWidth(320)
        self.ERRLOG2_HIGH_lineEdit.setFixedWidth(320)
        self.ERRLOG2_LOW_lineEdit.setFixedWidth(320)
        self.ERRLOG3_HIGH_lineEdit.setFixedWidth(320)
        self.ERRLOG3_LOW_lineEdit.setFixedWidth(320)
        self.FAULTINSTATUS0_HIGH_lineEdit.setFixedWidth(320)
        self.FAULTINSTATUS0_LOW_lineEdit.setFixedWidth(320)
        self.FAULTINSTATUS1_HIGH_lineEdit.setFixedWidth(320)
        self.FAULTINSTATUS1_LOW_lineEdit.setFixedWidth(320)
        ...
        self.NocErrlistComboBox.setPlaceholderText("选择NOC error type")
        ...
        self.NocErrlistComboBox.setFixedWidth(120)
        # 设置默认值
        self.NocErrlistComboBox.setCurrentIndex(-1)
    
        #self.NocErrlistComboBox.addItems(items)
        # 设置platformComboBox的编辑事件
        #self.platformComboBox.currentTextChanged.connect(logger.info)
        self.NocErrlistComboBox.currentIndexChanged.connect(self.NocErrlistComboBoxClicked)

        self.ERRLOG0_LOW_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG0_HIGH_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG1_LOW_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG1_HIGH_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG2_LOW_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG2_HIGH_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG3_LOW_lineEdit.setPlaceholderText("0x**")
        self.ERRLOG3_HIGH_lineEdit.setPlaceholderText("0x**")
        self.FAULTINSTATUS0_LOW_lineEdit.setPlaceholderText("0x**")
        self.FAULTINSTATUS0_HIGH_lineEdit.setPlaceholderText("0x**")
        self.FAULTINSTATUS1_LOW_lineEdit.setPlaceholderText("0x**")
        self.FAULTINSTATUS1_HIGH_lineEdit.setPlaceholderText("0x**")
        ...
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG0_LOW", "请输入0x开头的16进制数", self.ERRLOG0_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG0_HIGH", "请输入0x开头的16进制数", self.ERRLOG0_HIGH_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG1_LOW", "请输入0x开头的16进制数", self.ERRLOG1_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG1_HIGH", "请输入0x开头的16进制数", self.ERRLOG1_HIGH_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG2_LOW", "请输入0x开头的16进制数", self.ERRLOG2_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG2_HIGH", "请输入0x开头的16进制数", self.ERRLOG2_HIGH_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG3_LOW", "请输入0x开头的16进制数", self.ERRLOG3_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "ERRLOG3_HIGH", "请输入0x开头的16进制数", self.ERRLOG3_HIGH_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "FAULTINSTATUS0_LOW", "请输入0x开头的16进制数", self.FAULTINSTATUS0_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "FAULTINSTATUS0_HIGH", "请输入0x开头的16进制数", self.FAULTINSTATUS0_HIGH_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "FAULTINSTATUS1_LOW", "请输入0x开头的16进制数", self.FAULTINSTATUS1_LOW_lineEdit)
        self.addGroup("{}/images/Rocket.svg".format(resource_path), "FAULTINSTATUS1_HIGH", "请输入0x开头的16进制数", self.FAULTINSTATUS1_HIGH_lineEdit)
        self.addGroup("{}/images/Joystick.svg".format(resource_path), "Noc error list", "请选择NOC error类型", self.NocErrlistComboBox)
    ...
    def NocErrlistComboBoxClicked(self, index):
        logger.info("NocErrlist ComboBox Clicked: {}".format(index))
        # 打印当前选择的值
        logger.info("Current Index: {}".format(self.NocErrlistComboBox.currentText()))
    ...
    def platformComboBoxClicked(self, index):
        ...
        # 清空NocErrlistComboBox的值
        self.NocErrlistComboBox.clear()

        # 判断platformComboBox是否为空
        if self.platformComboBox.currentText() == "":
            self.showNoSelectPlatformFlyout()
        
        board = self.platformComboBox.currentText()
        boardfile = os.path.join(NOC_DECODE_TOOLS_PATH, "data_{}".format(board))

        # import boardfile
        try:
            sys.path.append(NOC_DECODE_TOOLS_PATH)
            exec("import data_%s as data" % board, globals())
        except Exception as e:
            print(str(e))
            error = "\nThe target %s is not supported\nTargets supported: " % board
            files = [f for f in os.listdir(NOC_DECODE_TOOLS_PATH) if os.path.isfile(f)]
            for f in files:
        ...
        # 获取board的配置信息
        noc_err_dict = data.data
        # 转成list
        noc_err_list = list(noc_err_dict.keys())
        logger.info(noc_err_list)
        # 设置NocErrlistComboBox的值
        self.NocErrlistComboBox.addItems(noc_err_list)
    ...
    def runButtonClicked(self):
        ...
        logger.info("ERRLOG0_LOW: {}".format(self.ERRLOG0_LOW_lineEdit.text()))
        logger.info("ERRLOG0_HIGH: {}".format(self.ERRLOG0_HIGH_lineEdit.text()))
        logger.info("ERRLOG1_LOW: {}".format(self.ERRLOG1_LOW_lineEdit.text()))
        logger.info("ERRLOG1_HIGH: {}".format(self.ERRLOG1_HIGH_lineEdit.text()))
        logger.info("ERRLOG2_LOW: {}".format(self.ERRLOG2_LOW_lineEdit.text()))
        logger.info("ERRLOG2_HIGH: {}".format(self.ERRLOG2_HIGH_lineEdit.text()))
        logger.info("ERRLOG3_LOW: {}".format(self.ERRLOG3_LOW_lineEdit.text()))
        logger.info("ERRLOG3_HIGH: {}".format(self.ERRLOG3_HIGH_lineEdit.text()))
        logger.info("FAULTINSTATUS0_LOW: {}".format(self.FAULTINSTATUS0_LOW_lineEdit.text()))
        logger.info("FAULTINSTATUS0_HIGH: {}".format(self.FAULTINSTATUS0_HIGH_lineEdit.text()))
        logger.info("FAULTINSTATUS1_LOW: {}".format(self.FAULTINSTATUS1_LOW_lineEdit.text()))
        logger.info("FAULTINSTATUS1_HIGH: {}".format(self.FAULTINSTATUS1_HIGH_lineEdit.text()))

        # 判断是否为空
        if self.ERRLOG0_LOW_lineEdit.text() == "" or self.ERRLOG0_HIGH_lineEdit.text() == "" or \
            self.ERRLOG1_LOW_lineEdit.text() == "" or self.ERRLOG1_HIGH_lineEdit.text() == "" or \
            self.ERRLOG2_LOW_lineEdit.text() == "" or self.ERRLOG2_HIGH_lineEdit.text() == "" or \
            self.ERRLOG3_LOW_lineEdit.text() == "" or self.ERRLOG3_HIGH_lineEdit.text() == "" or \
            self.FAULTINSTATUS0_LOW_lineEdit.text() == "" or self.FAULTINSTATUS0_HIGH_lineEdit.text() == "" or \
            self.FAULTINSTATUS1_LOW_lineEdit.text() == "" or self.FAULTINSTATUS1_HIGH_lineEdit.text() == "":
        ...
        decodefile = linuxPath2winPath(os.path.join(NOC_DECODE_TOOLS_PATH, "NoC_error_decode.py"))

        command = "{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}".format(PYTHON_BIN,
            decodefile, self.platformComboBox.currentText(), self.NocErrlistComboBox.currentText(), self.ERRLOG0_LOW_lineEdit.text(), self.ERRLOG0_HIGH_lineEdit.text(), self.ERRLOG1_LOW_lineEdit.text(), self.ERRLOG1_HIGH_lineEdit.text(), self.ERRLOG2_LOW_lineEdit.text(), self.ERRLOG2_HIGH_lineEdit.text(), 
            self.ERRLOG3_LOW_lineEdit.text(), self.ERRLOG3_HIGH_lineEdit.text(), self.FAULTINSTATUS0_LOW_lineEdit.text(), self.FAULTINSTATUS0_HIGH_lineEdit.text(), self.FAULTINSTATUS1_LOW_lineEdit.text(), self.FAULTINSTATUS1_HIGH_lineEdit.text())
...
class NocDecodeCardsInfo(ScrollArea):
...
class NocDecodeInterface:
    ...
    def addTab(self, routeKey, text, icon):
        ...
        # tab左对齐
        self.mainWindow.showInterface.addWidget(NocDecodeCardsInfo(routeKey=routeKey))
        self.mainWindow.showInterface.setCurrentWidget(self.mainWindow.showInterface.findChild(NocDecodeCardsInfo, routeKey))
        self.mainWindow.stackedWidget.setCurrentWidget(self.mainWindow.showInterface)
